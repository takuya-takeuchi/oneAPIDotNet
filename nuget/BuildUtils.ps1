class Config
{
   ...
   @{
      "oneAPIDotNet.Native"     = "oneAPIDotNetNative.dll";
   }
   
   static $BuildLibraryLinuxHash = 
   @{
      "oneAPIDotNet.Native"     = "liboneAPIDotNetNative.so";
   }
   
   static $BuildLibraryOSXHash = 
   @{
      "oneAPIDotNet.Native"     = "liboneAPIDotNetNative.dylib";
   }
   
   static $BuildLibraryIOSHash = 
   @{
      "oneAPIDotNet.Native"     = "liboneAPIDotNetNative.a";
   }
   ...
   #***************************************
   # Arguments
   #  %1: Root directory of oneAPIDotNet
...
function ConfigCPU([Config]$Config)
{
   ...
   $installoneAPIDir = $env:ONEAPI_ROOT
   ...
   if ($IsWindows)
   {
      ...
      # build path for each components
      $vpl = Join-Path $installoneAPIDir vpl | `
             Join-Path -ChildPath $oneAPIVersion | `
...
function ConfigUWP([Config]$Config)
{
   if ($IsWindows)
   {
      ...
      $oneAPIDir = $Config.GetoneAPIRootDir()
      $patchFullPath = Join-Path $nugetDir $patch
      $current = Get-Location
      Set-Location -Path $oneAPIDir
      Write-Host "Apply ${patch} to ${oneAPIDir}" -ForegroundColor Yellow
