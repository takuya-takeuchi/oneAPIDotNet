...
$oneAPIDotNetRoot = Split-Path $ScriptPath -Parents
$NugetPath = Join-Path $oneAPIDotNetRoot "nuget" | `
...
$oneAPIDotNetRoot = (Split-Path (Get-Location) -Parent)
$oneAPIDotNetSourceRoot = Join-Path $oneAPIDotNetRoot src
...
foreach ($BuildTarget in $BuildTargets)
{
   ...
   $Config = [Config]::new($oneAPIDotNetRoot, "Release", $target, $architecture, "desktop", $option)
   $libraryDir = Join-Path "artifacts" $Config.GetArtifactDirectoryName()
   $build = $Config.GetBuildDirectoryName($OperatingSystem)

   foreach ($key in $BuildSourceHash.keys)
   {
      $srcDir = Join-Path $oneAPIDotNetSourceRoot $key
